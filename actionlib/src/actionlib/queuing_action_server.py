...
## @class QueuingActionServer
## @brief The QueuingActionServer
...
class QueuingActionServer:
    ## @brief Constructor for a QueuingActionServer
    ...
    def register_goal_callback(self, cb):
        if self.execute_callback:
            rospy.logwarn("Cannot call QueuingActionServer.register_goal_callback() because an executeCallback exists. Not going to register it.")
        else:
    ...
    def internal_goal_callback(self, goal):
        ...
        try:
            ...
            rospy.logerr("QueuingActionServer.internal_goal_callback - exception %s", str(e))
            self.execute_condition.release()
    ...
    def internal_preempt_callback(self, preempt):
        with self.lock:
            rospy.logdebug("A preempt has been received by the QueuingActionServer")
        ...
        while (not rospy.is_shutdown()):
            ...
            if self.is_active():
                if not self.execute_callback:
                    rospy.logerr("execute_callback_ must exist. This is a bug in QueuingActionServer")
                    return
